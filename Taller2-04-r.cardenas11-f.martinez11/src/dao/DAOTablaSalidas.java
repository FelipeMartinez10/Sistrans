...
public class DAOTablaSalidas 
{
	...
	public ArrayList<Req7> darSalidasFiltradas(String fecha1, String fecha2, String nombre, String tipo) throws SQLException, Exception 
	{
		...
		String sql = "SELECT id_buque,NOMBRE ,FECHA, TIPO FROM( SELECT id_buque AS id,FECHA FROM SALIDA INNER JOIN EVENTO_PUERTO on SALIDA.ID_EVENTO = EVENTO_PUERTO.id_evento)  g INNER JOIN BUQUE on g.id = BUQUE.ID_BUQUE WHERE FECHA > '"+ f1 +"' AND  FECHA < '" + f2 +"' AND NOMBRE = '" + nombre + "' AND TIPO = '" + tipo + "'";
